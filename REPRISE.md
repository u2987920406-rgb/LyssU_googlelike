...
5. **Projets** — dessinés (`web/PASSE-DESIGN-PROJETS.md` v2) et **branchés le
   ...
   **déduit** (`isAuto`, dont l'id est le chemin) et `__no_project__`
   ...
   ✅ **`projects.archive` est branché** (2026-08-09) : « Archivés », jamais
   « Corbeille » — le drapeau se pose et se retire, **rien n'expire**.
...
6. **Où le fil travaille** — branché le 2026-08-09
   ...
   **Le lieu vient de la SESSION** : `conv.info` porte `cwd` ET `project`.
   ...
   ⚠ **« Travailler ici » NE FERME PLUS le fil ouvert.** Il appelait
   `resetSession()` d'abord — ce qui vide `conv.turns` : la conversation
   disparaissait de l'écran **sans un mot**, et l'état « deux dossiers à la
   fois » devenait inatteignable. La fermeture est désormais un choix nommé,
   dans le repli : « Ouvrir un fil là-bas ».
